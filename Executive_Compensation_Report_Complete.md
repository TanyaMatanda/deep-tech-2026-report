...
**Category 1: Legitimate Grant-Date Accounting** — Some apparent anomalies reflect genuine SEC disclosure mechanics. For example, multi-year performance grants may report full grant-date fair value in the award year even if vesting occurs over multiple periods. However, within the Summary Compensation Table itself, the Total column must mathematically equal the sum of components.
...
**Critical SCT Verification Rule**: Within the Summary Compensation Table (SCT), the "Total" column must equal the sum of all component columns by SEC regulation (Item 402(c) of Regulation S-K). **If component values in this report don't sum to the stated SCT Total, treat it as an extraction error, table-type mismatch (e.g., mixing SCT with Pay-versus-Performance data), or field mapping issue.** Verify all non-reconciling rows in the original DEF 14A and Compensation Discussion & Analysis before using the component figures numerically. Our automated extraction achieves approximately 70-80% accuracy; the remaining 20-30% require manual validation.
...
**Data Extraction Note**: Select biotechnology compensation records show component fields that don't reconcile with reported totals—likely reflecting inconsistent proxy filing structures or automated extraction limitations. These cases represent Category 2 anomalies (data collection artifacts) rather than Category 1 (SEC reporting mechanics). **When component sums don't reconcile to SCT Total, treat it as an extraction anomaly or table-type mismatch; verify in the DEF 14A and CD&A before using numerically.** Investors requiring precise component verification should consult original DEF 14A filings.

**Critical Note on SCT Mathematics**: Within the Summary Compensation Table, Total compensation is defined as the sum of all components by SEC rules (Item 402(c) of Regulation S-K). If extracted data shows components exceeding the stated total, this indicates extraction error, field mapping issues, or mixing of different disclosure tables (e.g., SCT vs. Pay-versus-Performance)—not legitimate reporting variation. Our methodology achieves approximately 70-80% accuracy in component attribution; anomalous records require manual DEF 14A verification.
...
- Common traits: Structurally aligned (equity-dominant >85%; ratio discipline <10x; multi-year vesting); most fall in $1-15M, with a small set of structurally mainstream high-pay leaders (e.g., Cisco, AMD)
